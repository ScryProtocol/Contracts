...
const REPLAY_TTL_SEC = Number(process.env.WEATHER_REPLAY_TTL_SEC || 2592000);
const REPLAY_MAX_ENTRIES = Number(process.env.WEATHER_REPLAY_MAX_ENTRIES || 50000);
const DIRECT_PAYMENT_ENABLED = NETS.some((net) => net.modes.includes("direct"));

if (DIRECT_PAYMENT_ENABLED && (!RPC_URL || !CONTRACT_ADDRESS)) {
  throw new Error("direct payments require RPC_URL and CONTRACT_ADDRESS");
}
if (!Number.isInteger(REPLAY_TTL_SEC) || REPLAY_TTL_SEC <= 0) {
  throw new Error("invalid WEATHER_REPLAY_TTL_SEC; expected positive integer seconds");
}
if (!Number.isInteger(REPLAY_MAX_ENTRIES) || REPLAY_MAX_ENTRIES <= 0) {
  throw new Error("invalid WEATHER_REPLAY_MAX_ENTRIES; expected positive integer");
}
...
function collectHubUrls() {
  return [...new Set(
    NETS
      .filter((net) => net.modes.includes("hub") && net.hubEndpoint)
      .map((net) => net.hubEndpoint)
  )];
}
...
class TtlMap {
  ...
  sweep() {
    ...
    for (const [key, entry] of this._map) {
      if (t > entry.exp) this._map.delete(key);
    }
  }
  _evictBatch() {
    const t = now();
...
async function validatePayment(rawHeader, ctx, expectedPath, expectedResource) {
  if (!rawHeader) return { ok: false, error: "no payment" };
  return ctx.verifyPayment(rawHeader, (invoiceId, paymentProof) => {
    const inv = ctx.invoices.get(invoiceId);
    if (!inv) return false;
    if (expectedPath && inv.path !== expectedPath) return false;
    if (expectedResource && inv.resource && inv.resource !== expectedResource) return false;
    if (paymentProof && inv.amount && String(paymentProof.amount || "") !== String(inv.amount)) return false;
    if (paymentProof && inv.asset && String(paymentProof.asset || "").toLowerCase() !== String(inv.asset).toLowerCase()) {
      return false;
    }
    if (paymentProof && inv.payee && String(paymentProof.payee || "").toLowerCase() !== String(inv.payee).toLowerCase()) {
      return false;
    }
    return inv;
  });
...
async function handle(req, res, ctx) {
  ...
  if (req.method === "GET" && u.pathname === "/weather") {
    ...
    if (!paymentHeader) {
      ...
      paymentResult = await validatePayment(paymentHeader, ctx, "/weather", resource);
      if (paymentResult.replayed) return sendJson(res, 200, paymentResult.response);
      if (!paymentResult.ok) return sendJson(res, 402, { error: paymentResult.error, retryable: false });
    ...
    const response = {
      ...
      ...(access ? { access } : {}),
      ...(paymentResult ? { receipt: { paymentId: paymentResult.paymentId, receiptId: randomId("rcpt"), acceptedAt: now() } } : {})
    };
    if (paymentResult) ctx.consumed.set(paymentResult.paymentId, response);
  ...
  if (req.method === "GET" && PATH_ASSET_PRICES[u.pathname]) {
    ...
    if (!paymentHeader) {
      ...
      paymentResult = await validatePayment(paymentHeader, ctx, u.pathname, resource);
      if (paymentResult.replayed) return sendJson(res, 200, paymentResult.response);
      if (!paymentResult.ok) return sendJson(res, 402, { error: paymentResult.error, retryable: false });
...
function createWeatherServer(options = {}) {
  ...
  const ctx = {
    ...
    consumed: new TtlMap(REPLAY_TTL_SEC, REPLAY_MAX_ENTRIES),
  ...
  if (CONTRACT_ADDRESS) {
    setDomainDefaults(NETS[0] && Number.isInteger(Number(NETS[0].chainId)) ? Number(NETS[0].chainId) : undefined, CONTRACT_ADDRESS);
  }
  ctx.verifyPayment = createVerifier({
    payee: PAYEE_ADDRESS,
    hubs: collectHubUrls(),
    confirmHub: true,
    chainId: NETS[0] && Number.isInteger(Number(NETS[0].chainId)) ? Number(NETS[0].chainId) : undefined,
    rpcUrl: RPC_URL,
    contractAddress: CONTRACT_ADDRESS,
    requireDirectOnChain: DIRECT_PAYMENT_ENABLED,
  ...
  server.on("close", () => {
    ...
    if (ctx.verifyPayment && typeof ctx.verifyPayment.close === "function") ctx.verifyPayment.close();
  });
  return server;
}
